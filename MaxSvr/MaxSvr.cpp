...
{
	cout << "[客户加入连接]: " << inet_ntoa(pContext->stRemoteAddr.sin_addr) << endl;
	SendBuffer(pContext, "Welcode join IOCP Server");
}

void MaxSvr::OnClientClose(PIOContext pContext, PIOBuffer pBuffer)
{
	cout << "[客户关闭连接]: " << inet_ntoa(pContext->stRemoteAddr.sin_addr) << endl;
}

void MaxSvr::OnConnectErro(PIOContext pContext, PIOBuffer pBuffer,int nError)
{
	cout << "[连接发生错误]: " << inet_ntoa(pContext->stRemoteAddr.sin_addr) << ": " << nError << endl;
}

void MaxSvr::OnSendFinish(PIOContext pContext, PIOBuffer pBuffer)
{
	cout << "[发送操作完成]: " << inet_ntoa(pContext->stRemoteAddr.sin_addr) << ": " << pBuffer->szBuffer << endl;
}

void MaxSvr::OnRecvFinish(PIOContext pContext, PIOBuffer pBuffer)
{
	cout << "[接收操作完成]: " << inet_ntoa(pContext->stRemoteAddr.sin_addr) << ": " << pBuffer->szBuffer << endl;
}
